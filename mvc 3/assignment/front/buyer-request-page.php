...
if(isset($_GET['allowd'])){
	...
	WHERE NoteID=$updatenoteid AND Downloader=$buyerid";
	...
	$mail->addAddress("$buyer", 'User');                   
	$mail->addReplyTo("$SystemEmail", 'Information');



	$mail->isHTML(true);                                  
	$mail->Subject = "$user_Fn $user_Ln Allow you to download note";
	$mail->Body    = "Hello,$buyer_Fn $buyer_Ln<br><br>We Would like to inform you that $user_Fn $user_Ln Allows you to Download a note please login and see my download tab to download note.<br><br>
	Regards,<br>NotesMarketplace";
...
$query = "SELECT * FROM downloads WHERE Seller=$sellerid AND IsSellerHasAllowedDownload=0 ORDER BY ID DESC"; 
...
$selltype 		= $row['IsPaid'];
$buyer 			= tabledata('users','Email','ID',$buyerid);
$Phoneno		= tabledata('userprofile','Phonenumber','UserID',$buyerid);
$Phonecode		= tabledata('userprofile','Phonenumber–CountryCode','UserID',$buyerid);
if($selltype == 0){
	$selltype 	= "Free";
} else {
	$selltype 	= "Paid";
