...
if(isset($_POST['submit'])){
	...
	$db_id 		= $row['ID'];
		...
		if($newpwd === $confirmpwd && !empty($newpwd)){
				...
				$massage = "<P style='	color: green;font-size: 16px;'>Your password change successfully.</P>";

	

			} else {
				$massage = "<P style='	color: red;font-size: 16px;'>Password you entered don't match.<br>try again.</P>";
			}
		} else {
			$massage = "<P style='	color: red;font-size: 16px;'>Password must contain at least one  number and one uppercase and lowercase letter, and at least 6 or more characters.<br>try again.</P>";
		}

		} else {
			$massage = "<P style='	color: red;font-size: 16px;'>Your old password is incorrect.</P>";
		}
  }
} else {
	redirect();
