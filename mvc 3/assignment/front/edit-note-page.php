...
$Status = tabledata('sellernotes','Status','ID',$noteid);	
	
if($Status != 8){echo '<script>alert("You can not edit this note");window.location.href="search-page.php";</script>';}	
if(isset($_GET['publish'])){
$reportednotetitle 	= tabledata('sellernotes','Title','ID',$noteid);	
$seller_id 			= tabledata('sellernotes','SellerID','ID',$noteid);	
$seller_Fn 			= tabledata('users','FirstName','ID',$seller_id);
$seller_Ln 			= tabledata('users','LastName','ID',$seller_id);
	...
	$mail->Subject = "$seller_Fn $seller_Ln Send his note for review";
	$mail->Body    = "Hello Admins,<br><br>
						We want to inform you that $seller_Fn $seller_Ln send his note $reportednotetitle for review. Please look at the notes and take required actions.<br><br>Regards,<br>Notes Marketplace ";
	if(!$mail->send()) {
...
redirect("edit-note-page.php?editnote=$noteid");
...
<body>
	...
	<div class="background-img">
		...
		<p class="text-center">Edit Notes</p>
	</div>
								...
								<div class="col-md-12 col-sm-12">
									<div class="add-border text-center">
										<label for="displaypicture">
											...
											<p id="textdisplaypic"><?php echo $e_displaypicture; ?></p>
										</label>
...
<?php
	$query = "SELECT * FROM `sellernotesattachements` WHERE `NoteID`='$noteid' ";
	$totl_row_count_query = mysqli_query($connection,$query);
	$c=mysqli_num_rows($totl_row_count_query);																
?>
											
											<p id="remove-text" style="color: #333333;"><?php echo $c; ?> Notes</p>
										</label>
										<input type="file" name="upload[]" id="upnotes" class="upload" accept="application/pdf, application/vnd.ms-excel" style="display:none;" multiple="multiple" />
					...
					<div id="add-notes-btn">
						<div class="row">
							...
							<div class="col-md-4 col-sm-4 col-4">
								<a onclick="return confirm('Are you sure you want to publish this note?')" href="<?php echo "edit-note-page.php?editnote=$noteid&publish=1";?>"><button type="button" name="publish" class="btn btn-block btn-general btn-publish">PUBLISH</button></a>
