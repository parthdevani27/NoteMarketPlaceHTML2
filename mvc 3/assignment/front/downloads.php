...
if(!isset($_SESSION['ID'])){
	...
	$user_Fn = $_SESSION['firstname'];
	$user_Ln = $_SESSION['lastname'];
}
...
$reportednotetitle 	= tabledata('sellernotes','Title','ID',$reportnoteid);	
$seller_id 			= tabledata('sellernotes','SellerID','ID',$reportnoteid);	
...
$seller_Ln 			= tabledata('users','LastName','ID',$seller_id);
	...
	$mail->isHTML(true);                                  
	$mail->Subject = "$user_Fn $user_Ln Reported an issue for $reportednotetitle";
	$mail->Body    = "Hello Admins,<br><br>
						We want to inform you that $user_Fn $user_Ln reported an issur for $seller_Fn $seller_Ln's note with title $reportednotetitle. Plesae look at note and take required action.<br><br>Regards,<br>Notes MarketPlace ";
	if(!$mail->send()) {
...
if(isset($_GET['download'])){
	...
	while($row = mysqli_fetch_assoc($download_path_query)){
		...
		$userid = $_SESSION['ID'];
		$query = "UPDATE `downloads` SET `IsAttachmentDownloaded`=1,`AttachmentDownloadedDate`=now() WHERE `NoteID`=$downloadnoteid AND `Downloader`=$userid";
			$run_query= mysqli_query($connection,$query);
			confirmq($run_query);
		}
	$zipname = time().".zip";
...
while($row = mysqli_fetch_assoc($downloadnotes_query)){
	...
	$buyer_Fn 		= tabledata('users','FirstName','ID',$buyerid);
	$buyer_Ln 		= tabledata('users','LastName','ID',$buyerid);
...
?>
									...
									<tr>
										...
										<td><?php echo $buyer_Fn.' '.$buyer_Ln; ?></td>
										...
										<td>
											<div class="dropdown">
												...
												<div class="dropdown-content setwidth">
													...
													<a onclick="return confirm('Are you sure you want mark this report as spam?you cannot update it later.');" href="downloads.php?report=<?php echo $noteid; ?>">Report as Inappopriate</a>
												</div>
											</div>
										</td>
									</tr>
