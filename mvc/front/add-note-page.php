...
if(isset($_POST['save'])){
	...
	$query = "SELECT ID FROM sellernotes ORDER BY ID DESC";
	$last_note_id_query = mysqli_query($connection,$query);
	confirmq($last_note_id_query);
	 $row = mysqli_fetch_assoc($last_note_id_query);
	 $lastnoteid = $row['ID'];
	if (!file_exists("upload/member/$sellerid/$lastnoteid/attachment")) {
    mkdir("upload/member/$sellerid/$lastnoteid/attachment", 0777, true);
	}
...
($sellerid, 8, 1, 'ar', now(), '$title', $category, '$displaypicture', $notetype, $numofpage, '$description', '$institution',$country, '$courcename', '$courcecode', '$professor',$sellfor, $sellprice, '$noteprevie', NULL, NULL, NULL, NULL, b'1')";
